...
class MLX90393Hal {
    public:
        ...
        virtual bool transceive(const uint8_t *request, size_t request_size, uint8_t *response, size_t response_size) = 0;
        virtual bool read_drdy_pin() = 0;
        virtual void sleep_millis(uint32_t millis) = 0;
        virtual void sleep_micros(uint32_t micros) = 0;
    protected:
